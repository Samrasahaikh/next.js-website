import Image from "next/image";
import React from "react";

interface propsType {
  img: string;
  title: string;
  mainTitle: string;
  price: string;
}

const Slide: React.FC<propsType> = ({ img, title, mainTitle, price }) => {
  return (
    <div className="outline-none border-none relative">
      <div
        className="absolute left-[30px] md:left-[70px] max-w-[250px] sm:max-w-[250px] top-[50%]
        -translate-y-[50%] space-y-2 font-sans lg:space-y-4 bg-slate-300 sm:bg-transparent p-4 sm:p-0 rounded-lg sm:rounded-none"
      >
        <h3 className="text-pink-800 text-[24px] lg:text-[28px] font-serif">
          {title}
        </h3>
        <h2 className="text-black text-[26px] md:text-[30px] lg:text-[44px] font-bold leading[2.3] font-serif">
          {mainTitle}
        </h2>

        <h3 className="text-[24px] text-gray-600">
          starting at{" "}
          <b className="text-[20px] md:text-[24px] lh:text-[30px] font-sans">
            {price}
          </b>
          .00
        </h3>
        <div
          className="bg-pink-800 text-white text-[14px] md:[16px] p-2 px-4 rounded-lg *
            inline-block cursor-pointer hover:bg-pink-600 font-sans"
        >
          Shope Now
        </div>
      </div>

      <Image
        className="w-[100%] h-[300px] md:h-auto rounded-xl object-cover object-right
md:object-left-bottom"
        src={img}
        alt="banner"
        width={2000}
        height={2000}
      />
    </div>
  );
};

export default Slide;
